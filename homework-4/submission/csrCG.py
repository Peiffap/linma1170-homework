...
# mpltools needs to be installed
...
def convergence_analysis(prec='report'):
    ...
    RCMK reordering was used to speed up plotting for larger matrices.
...
#    Needs the mpltools package
#    annotation.slope_marker((10000, 103), (1, 2),
...
#                        poly_kwargs={'facecolor': (1, 0.8, 0.73)})
    ...
    x, res = csrCG(sA, iA, jA, b, 1e-12, False)
    x_prec, res_prec = csrCG(sA, iA, jA, b, 1e-12, True)
    plt.loglog(range(len(res)), res, '-b.', range(len(res_prec)), res_prec, '-r.')
    ...
    for i in data_gap:
        ...
        size[index] = len(A)
        sA, iA, jA = CSRformat(A)
        r = RCMK(iA, jA)  # Band size goes down so the code runs faster
        a_rcmk = (A[:, r])[r, :]
        b = b[r]
        sA, iA, jA = CSRformat(a_rcmk)
        x, res, n_iter[index] = csrCG(sA, iA, jA, b, 1e-6, False, True)
    ...
    plt.loglog(size, n_iter, 'bo', size, n_iter_prec, 'ro', size, size, '--g')
